...
1. 在 Gmail Draft 中搜索 `[INV-MONTHLY][HANDOFF]`。新格式 Subject 必须匹配：`^\[INV-MONTHLY\]\[HANDOFF\]\s+\d{4}-\d{2}(?:\s+R\d+)?$`。旧格式 `[INV-MONTHLY][HANDOFF] Monthly-to-Daily Research Handoff v*` 只作向后兼容；一旦存在有效且明确取代它的新格式 Handoff，不再选旧格式。
...
11. 每个固定章节必须包含与当日信息相符的非空正文；信息不足时写明“过去24小时无足以改变判断的高价值新证据”及仍需验证的指标，不得删除章节、用机器状态码替代章节，或把正文压缩为 Audit 状态。
...
4. 创建 Daily 前精确搜索 `[INV-AUDIT][DAILY] YYYY-MM-DD` 并检查实际 Subject：0封且存在正式事件才可创建；1封报告 `AUDIT_LEDGER_ALREADY_EXISTS`；多于1封报告 `AUDIT_LEDGER_DUPLICATE`。不得修改、覆盖、删除、合并或再创建冲突记录。
